package com.itcareer.media.jwt;

import com.itcareer.media.utils.ZipUtils;
import lombok.Data;
...
@Slf4j
@Data
public class ItcareerJwt implements Serializable {
    ...
    public static ItcareerJwt decode(String input){
        ItcareerJwt result = null;
        try {
            String[] items = ZipUtils.unzipString(input).split(DELIM,11);
            if(items.length >= 10){
                result = new ItcareerJwt();
